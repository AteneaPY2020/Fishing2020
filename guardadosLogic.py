...
class guardadosLogic(Logic):
    ...
    def guardar(self, id_inversionista, id_producto):
        ...
        sql = (
            "insert into heroku_c9cfc4eae6e8f6a.guardado (id, id_inversionista, id_producto) "
            + "values (0, %s, %s);"
        )
    ...
    def deleteGuardado(self, id_inversionista, id_producto):
        database = self.get_databaseXObj()
        sql = f"delete from heroku_c9cfc4eae6e8f6a.guardado where guardado.id_inversionista = {id_inversionista} and guardado.id_producto = {id_producto};"
        rows = database.executeNonQueryRows(sql)
        return rows

    def checkGuardado(self, id_inversionista, id_producto):
        database = self.get_databaseXObj()
        sql = f"select * from heroku_c9cfc4eae6e8f6a.guardado where guardado.id_inversionista = {id_inversionista} and guardado.id_producto = {id_producto};"
        print(sql)
        data = database.executeQuery(sql)
        counter = 0
        for item in data:
